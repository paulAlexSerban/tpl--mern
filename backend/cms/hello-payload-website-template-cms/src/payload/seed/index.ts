...
export const seed = async (payload: Payload): Promise<void> => {
    ...
    const [technologyCategory, newsCategory, financeCategory, designCat, softwareCat, engineeringCat] =
        await Promise.all([
            await payload.create({
    ...
    const project1Doc = await payload.create({
        collection: 'projects',
        data: JSON.parse(
            JSON.stringify({ ...project1, categories: [designCat.id] }).replace(/"\{\{IMAGE\}\}"/g, image2ID)
        ),
    });
    ...
    const project2Doc = await payload.create({
        collection: 'projects',
        data: JSON.parse(
            JSON.stringify({ ...project2, categories: [softwareCat.id] }).replace(/"\{\{IMAGE\}\}"/g, image2ID)
        ),
    });
    ...
    const project3Doc = await payload.create({
        collection: 'projects',
        data: JSON.parse(
            JSON.stringify({ ...project3, categories: [engineeringCat.id] }).replace(/"\{\{IMAGE\}\}"/g, image2ID)
        ),
    });
